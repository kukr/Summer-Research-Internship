...
void
	...
	//	uint8 no_to_trav = data[6]-1;
		...
		//{
			...
			for(int i = 0;i<3;i++)
			{
				if(((N[i].addr)%256 == rem[15]))
				{
					phermone[(N[i].addr)%256] = (1-rho)*phermone[(N[i].addr)%256] + delta_tk;
	...
	else if((data[0] == 202) )
	{
		...
		//uint8 for_101(0);
		uint8 do_not_cou[10],do_c_neigh(0);
		...
		if((dele != TRUE))
		{
			...
			for(uint8 j=0;j<3;j++)
			{
				...
				else
				{
					...
					for(uint8 i = 4+no_o_ho;no_o_ho>0;i--)
					{
		...
		if(dele != TRUE)
		{
			...
			//{
				...
				F[15] = C1;
				IPv6Address target(F);
				os_.debug("Forwarding forward ant to %x with hops %d",C1,data[4]+1);
				target.interface_id_ = 0;
				IpAddress remote_host = target;
				uint16 length = 16;
				uint8 answer[16];
				...
				uint16 voltage = cm_->supply_voltage();
				float avg_vol = answer[2]/10.0 + answer[3]/1000.0;
				answer[4] = answer[4]+1;
